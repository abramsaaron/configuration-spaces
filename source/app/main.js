...
function initgraphicsForGraph(w, h) {
  ...
  if (verbose) console.log(graphicsForGraph);
...
function initGUI() {
  ...
  pane.addInput(parameters, "showInfo").on("change", (e) => {
    if (e.value) {
      infoDiv.show();
    } else {
      infoDiv.hide();
  ...
  let graphPicker = globalUI.add(parameters, "graphType", ["C(20)", "K(1,1)", "K(1,2)", "K(1,3)", "K(1,4)", "K(1,5)", "K(1,6)", "K(1,7)", "K(1,8)", "K(1,9)", "K(1,10)", "K(2,2)", "K(2,3)", "K(2,4)", "K(2,5)", "K(2,6)", "K(3,3)", "K(3,4)", "K(4,4)", "K(2)", "K(3)", "K(4)", "K(5)", "K(6)", "K(7)", "K(8)", "C(2)", "C(3)", "C(4)", "C(5)", "C(6)", "C(7)", "W(4)", "W(5)", "W(6)", "W(7)", "W(8)", "W(9)", "W(10)", "custom"]).name("Choose graph").listen();
  ...
  let showGraphToggle = globalUI.add(parameters, "showGraph").name("Show graph");
  showGraphToggle.onChange(function (value) {
    updateURL();
  });
  let showConfigurationspaceToggle = globalUI.add(parameters, "showConfigurationspace").name("Show configuration space");
  showConfigurationspaceToggle.onChange(function (value) {
    updateURL();
  });
  let showRobotsToggle = globalUI.add(parameters, "showRobots").name("Show robots");
  showRobotsToggle.onChange(function (value) {
    updateURL();
  });
  let showInfoToggle = globalUI.add(parameters, "showInfo").name("Show info");
  showInfoToggle.onChange(function (value) {
    if (value) {
      infoDiv.show();
    } else {
      infoDiv.hide();
  ...
  if (viewingStyle === "dual") {
    globalUI.add(parameters, "syncView").name("Sync views");
  }
  ...
  visualGUISpace.add(parameters, "edgeWidthConfigSpace", 0, 10).name("Edge width in space");
  ...
  graphGUI.add(graph.graphLayout, "edgelength", 0, 400).name("Target edge length");
  graphGUI.add(graph.graphLayout, "graphEdgeForce", 0, 0.1).name("Edge force");
  ...
  graphGUI.add(graph.graphLayout, "planarForce", 0, 0.15).name("Planar force").listen();
  ...
  configGUI.add(configuration_space.graphLayout, "firstCoordinateForce", 0, 0.1).name("force for First coordinate edge");
  ...
  configGUI.add(configuration_space.graphLayout, "secondCoordinateForce", 0, 0.1).name("force for Second coordinate edge");
  //
  configGUI.add(configuration_space.graphLayout, "firstCoordinateMirrorEdgeLength", 1, 1000).name("First coordinate mirror target edge length");
  configGUI.add(configuration_space.graphLayout, "firstCoordinateMirrorForce", 0, 0.1).name("force for First coordinate mirror edge");
  configGUI.add(configuration_space.graphLayout, "secondCoordinateMirrorEdgeLength", 1, 1000).name("Second coordinate mirror target edge length");
  configGUI.add(configuration_space.graphLayout, "secondCoordinateMirrorForce", 0, 0.1).name("force for Second coordinate mirror edge");
  //
  configGUI.add(configuration_space.graphLayout, "coordinatePreference", -0.1, 0.1).name("coordinatePreference");
  configGUI.add(configuration_space.graphLayout, "extraCenterPreference", 0.0, 0.1).name("extraCenterPreference");
  //
...
function draw() {
  ...
  if (takeScreenshotConfigSpace) {
    ...
    saveCanvas(configuration_space.graphLayout.graphics, makeFileName("-configspace.png"));
  }
...
function makeFileName(postString) {
  let time = str(year()) + ("0" + str(month())).slice(-2) + ("0" + str(day())).slice(-2) + "_" + ("0" + str(hour())).slice(-2) + ("0" + str(minute())).slice(-2) + ("0" + str(second())).slice(-2);
  return time + "-" + parameters.graphType + postString;
}
...
function initGraph(graphType) {
  ...
  if (graphType === "custom" && customGraph !== undefined) {
...
class GraphLayout {
  constructor(source, graphics, layout3D) {
    ...
    this.firstCoordinateMirrorEdgeLength = 100;
    this.secondCoordinateMirrorEdgeLength = 100;

    this.graphEdgeForce = 0.01;
    this.firstCoordinateForce = 0.05;
    this.secondCoordinateForce = 0.01;
    this.firstCoordinateMirrorForce = 0.0;
    this.secondCoordinateMirrorForce = 0.0;
...
class Node {
  ...
  update(nodes) {
    ...
    if (this.graphLayout.source.type === "graph") {
      let coh = this.cohese(this.graphLayout.edgelength, this.graphLayout.graphEdgeForce, this.neighbors);
      ...
      let planarForceAddition = this.seek(this.graphLayout.planarForce, planarNodePosition);
      this.acceleration.add(planarForceAddition);
    } else {
      // This is for the configuration space.
      ...
      // let cohAgraph = this.cohese(
      //   this.graphLayout.firstCoordinateMirrorEdgeLength,
      //   this.graphLayout.coordinatePreference + this.graphLayout.firstCoordinateMirrorForce,
      //   graph.graphLayout.nodes.filter((node) => node.label === this.label[0])
      // );

      // let cohBgraph = this.cohese(
      //   this.graphLayout.secondCoordinateMirrorEdgeLength,
      //   this.graphLayout.coordinatePreference + this.graphLayout.secondCoordinateMirrorForce,
      //   graph.graphLayout.nodes.filter((node) => node.label === this.label[1])
      // );

      let cohAgraph = this.seek(this.graphLayout.coordinatePreference + this.graphLayout.firstCoordinateMirrorForce, graph.graphLayout.getNode(this.label[0]).position);
      let cohBgraph = this.seek(this.graphLayout.coordinatePreference + this.graphLayout.secondCoordinateMirrorForce, graph.graphLayout.getNode(this.label[1]).position);

      graph.graphLayout.nodes.forEach((node) => {
        if (node.applyExtraCenterForce) {
          if (this.label[0] === node.label) {
            cohAgraph = cohAgraph.add(this.seek(this.graphLayout.extraCenterPreference, node.position));
          }
          if (this.label[1] === node.label) {
            cohBgraph = cohBgraph.add(this.seek(this.graphLayout.extraCenterPreference, node.position));
          }
        }
      });

      // let sepB = this.separate(graph.graphLayout.nodes.filter((node) => node.label === this.label[1]));
  ...
  show(g) {
    ...
    if (this.graphLayout.layout3D) {
      ...
      if (parameters.sphereView) {
        ...
        g.stroke(0);
        g.strokeWeight(1.0);
        g.ellipse(0, 0, parameters.nodeSize, parameters.nodeSize);
      }
      g.pop();
    }
    ...
    if (parameters.showText) {
      ...
      if (this.graphLayout.layout3D) {
        ...
        g.rotateY(-rotXYZ[1]);
        g.rotateZ(-rotXYZ[2]);
        g.translate(parameters.labelX, parameters.labelY, parameters.labelZ);
...
class Edge {
  ...
  show(g) {
    ...
    if (this.edgeType === "graphEdge") {
      g.strokeWeight(parameters.edgeWidthGraph);
    } else {
...
class Robot {
  ...
  setAmount(nextAmount) {
    ...
    if (this.amount === 0) {
      if (verbose) console.log("this.amount === 0.0");
      if (verbose) console.log(this.nodeFrom.label + " " + this.nodeTo.label);
      if (verbose) console.log(this.graph.graphLayout.getEdge(this.nodeFrom.label, this.nodeTo.label, false));
    ...
    if (this.amount === 1.0) {
      if (verbose) console.log("this.amount === 1.0");
      if (verbose) console.log(this.nodeFrom.label + " " + this.nodeTo.label);
      if (verbose) console.log(this.graph.graphLayout.getEdge(this.nodeFrom.label, this.nodeTo.label, false));
      if (this.nodeFrom !== this.nodeTo) {
        if (verbose) console.log("resetting!");
        this.graph.graphLayout.getEdge(this.nodeFrom.label, this.nodeTo.label, false).owner = undefined;
        this.amount = 0;
...
function mousePressed(e) {
  ...
  if (verbose) console.log("mouse pressed");
  if (verbose) console.log(areWeOnTheLeft);
  if (verbose) console.log(parameters.mode);
  if (parameters.mode === "Move" || parameters.mode === "Edit") {
    // areWeOnTheLeft = mouseX < windowWidth / 2 && viewingStyle === "dual" && e.target.nodeName === "CANVAS";
...
function ourMouseDragged() {
  if (verbose) console.log("ourMouseDragged");
  if (parameters.mode === "Move") {
    reheat();
...
function keyPressed() {
  ...
  else if (key === "w") writeToFile();
  else if (key === "r") readFromFile("graph.txt");
  else if (key === "m") {
    graphicsForConfigurationSpace.easycam.removeMouseListeners();
    graphicsForGraph.easycam.removeMouseListeners();
  } else if (key === "M") {
...
function updateURL() {
  ...
  if (verbose) console.log(history.pushState);
  if (history.pushState) {
    let newurl =
      ...
      "?" +
      (parameters.graphType === "custom" ? "graph=custom" + "&" + "nodes=" + graph.nodes + "&" + "edges=" + edgesToString(graph.edges) : "graph=" + parameters.graphType) +
      "&" +
      "view=" +
      viewingStyle +
      "&" +
...
function setParametersFromURL() {
  ...
  if (graphStr !== undefined && graphStr !== "custom") {
    ...
    if (verbose) console.log(edgesStr);
...
function updateInfoString() {
  ...
  graphParameters["maxspeed"] = graph.graphLayout.maxspeed;
  ...
  configSpaceParameters["firstCoordinateMirrorEdgeLength"] = configuration_space.graphLayout.firstCoordinateMirrorEdgeLength;
  configSpaceParameters["firstCoordinateMirrorForce"] = configuration_space.graphLayout.firstCoordinateMirrorForce;
  configSpaceParameters["secondCoordinateMirrorEdgeLength"] = configuration_space.graphLayout.secondCoordinateMirrorEdgeLength;
  ...
  configSpaceParameters["maxspeed"] = configuration_space.graphLayout.maxspeed;
...
function writeToFile() {
  // Write to file.
  saveStrings(infoStrings, makeFileName("-parameters.txt"));
}
...
function readFromString(strings) {
  ...
  graph.graphLayout.maxspeed = graphParameters["maxspeed"];
  ...
  configuration_space.graphLayout.firstCoordinateMirrorEdgeLength = configSpaceParameters["firstCoordinateMirrorEdgeLength"];
  configuration_space.graphLayout.firstCoordinateMirrorForce = configSpaceParameters["firstCoordinateMirrorForce"];
  configuration_space.graphLayout.secondCoordinateMirrorEdgeLength = configSpaceParameters["secondCoordinateMirrorEdgeLength"];
  ...
  configuration_space.graphLayout.maxspeed = configSpaceParameters["maxspeed"];
  ...
  graphicsForConfigurationSpace.easycam.setState(JSON.parse(cameraStateString));
