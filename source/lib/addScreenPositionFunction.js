...
function addScreenPositionFunction(p5Instance) {
  ...
  p.screenPositionTo3Dposition = function (x, y, z) {
    // TODO
    let v = p.createVector(x, y, z);

    // Calculate the ModelViewProjection Matrix.
    let mvp = p._renderer.uMVMatrix.copy().mult(p._renderer.uPMatrix);
    // console.log("p._renderer.uMVMatrix");
    // console.log(p._renderer.uMVMatrix);
    // console.log("p._renderer.uPMatrix");
    // console.log(p._renderer.uPMatrix);
    // print("mvp");
    // print(mvp);
    ...
    let p5ID = p5.Matrix.identity();

    console.log("mvp");
    console.log(mvp);

    let mInv = mvp.copy().invert(mvp.copy());
    ...
    // Transform the vector to Normalized Device Coordinate.
    let vNDC = multMatrixVector(mInv, v);
